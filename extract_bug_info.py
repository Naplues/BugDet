...
def get_bug_id_list(project):
    ...
    :return:
    """
    bug_reports = read_data_from_file(rf'{data_collection_path}/BugReport/{project}.csv')
...
def main_step_parse_diff_to_get_commit_bug_files_map(project, analysis_file_path):
    ...
    if not os.path.exists(f'{analysis_file_path}/BFC_commit_bug_files.pk'):
        ...
        commit_bug_files_map, content_of_prev_commit, prev_commit_is_bug, duplicated = [], [], False, False

        # Considering all bug id as matched patterns
        bug_id_list = get_bug_id_list(project)

        # Parsing diff.txt file line by line
        for line in read_data_from_file(rf'{analysis_file_path}/diff.txt'):
            ...
            if len(tmp) != 3:
                ...
                commit_bug_files_map.append(content_of_prev_commit) if prev_commit_is_bug else None
                ...
                prev_commit_is_bug, bug_id = match_bug_pattern(project, commit_log, bug_id_list)
                if not prev_commit_is_bug:
                    continue
                content_of_prev_commit = [commit_id, bug_id, [], commit_log]

            # When processing file change lines, add changed files into list
            if len(tmp) == 3 and prev_commit_is_bug and tmp[2].endswith('.java'):
        ...
        for instance in commit_bug_files_map:
            commit_id, buggy_files = instance[0], instance[2]
            if commit_id not in all_bug_fixing_commit and len(buggy_files) > 0:
...
def main_step_export_bug_fixing_commit_diff_file(analysis_file_path):
    """
    ...
    for index in range(len(commit_bug_files_map)):
        ...
        print(f'Exporting {index + 1}/{len(commit_bug_files_map)} diff file.') if (index + 1) % 500 == 0 else None
...
def main_step_parse_diff_file_to_get_bug_inducing_commit(analysis_file_path):
    ...
    """
    if not os.path.exists(f'{analysis_file_path}/bug_commits_lines_info.csv'):

        result_text = 'bug_fixing_commit,src_files,bug_line_in_last_commit,bug_inducing_commit\n'
        commit_bug_files_map = load_pk_result(f'{analysis_file_path}/BFC_commit_bug_files.pk')
        for index in range(len(commit_bug_files_map)):
            print(f'{index + 1}/{len(commit_bug_files_map)}')

            bug_fixing_commit = commit_bug_files_map[index][0]  # bug fixing commit

            os.system(f'git reset --hard {bug_fixing_commit}~1')  # 定位到BFC的前一个commit(i.e., 包含bug的最后一个commit)
            ...
            for bug_file in bug_file_info:
                buggy_lines = bug_file.hunk_info['d']  # The buggy lines deleted in the BFC commit
                buggy_file = bug_file.src_file[2:]  # remove a/ to get the original file path
                # Skip if it is not a java file or there is no deleted lines or it is a test file
                if not buggy_file.endswith('.java') or len(buggy_lines) == 0 or is_test_file(buggy_file):
                    continue

                # The bug lines are introduced in their corresponding commits
                for instance in identify_bug_inducing_commit(buggy_file, buggy_lines, analysis_file_path):
                    line_in_previous_commit = instance[0]
                    bug_inducing_commit = instance[1]

                    result_text += f'{bug_fixing_commit},{buggy_file},{line_in_previous_commit},{bug_inducing_commit}\n'

        save_data_to_file(f'{analysis_file_path}/bug_commits_lines_info.csv', result_text)
        os.system(f'git reset --hard {commit_all[0]}')

    print('===== The Bug-Inducing Commit (BIC) commit has been identified.\n')
